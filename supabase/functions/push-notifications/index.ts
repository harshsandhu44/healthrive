...
const VAPID_SUBJECT =
  Deno.env.get("VAPID_SUBJECT") || "mailto:support@vylune.com";
...
async function encryptPayload(payload: string): Promise<ArrayBuffer> {
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `Push notification sent: ${successCount}/${subscriptions.length} successful`,
    );
